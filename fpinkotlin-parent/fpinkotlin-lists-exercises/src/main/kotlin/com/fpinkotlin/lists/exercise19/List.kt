...
sealed class List<out A> {
    ...
    fun filterViaFoldLeft(p: (A) -> Boolean): List<A> = foldLeft(Nil) {
            acc: List<A> -> { h: A -> if (p(h)) Cons(h, acc) else acc } }

    fun filterViaCoFoldRight(p: (A) -> Boolean): List<A> = coFoldRight(Nil) {
            h: A -> { acc: List<A> -> if (p(h)) Cons(h, acc) else acc } }
